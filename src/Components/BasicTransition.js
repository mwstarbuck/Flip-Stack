...
const BasicTransition = () => {
  ...
  const transition = useTransition(/*isVisible*/ items, {
    ...
    enter: item => async (next) => {
      await next({ y: item.y, opacity: 1, delay: item.delay }); // use callback next when moving several items
      await next({ x: 0 }); // second cal of next splits the animation into 2 steps
